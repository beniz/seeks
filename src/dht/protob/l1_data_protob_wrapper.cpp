    ...
    if (!vt)
      {
        ...
        throw dht_exception(DHT_ERR_MSG,"Can't serialize a null vnode data table");
      }
    if (!vt->SerializeToOstream(&out))
      throw dht_exception(DHT_ERR_MSG,"Can't serialize vnode_table to ostream");
  }
    ...
    if (!vt)
      {
        ...
        throw dht_exception(DHT_ERR_MSG, "can't deserialize a null vnode data table");
      }
    if (!vt->ParseFromIstream(&in))
      throw dht_exception(DHT_ERR_MSG,"Can't ParseFromIstream vnode_table");
  }
