...
class adfilter_element : public filter_plugin
{
  ...
  private:
    // Methods
    static void filter(char **ret, std::vector<struct adr::adb_rule> *rules);                 // Filter page
    static xmlNodePtr filter_node(xmlNodePtr node, std::vector<struct adr::adb_rule> *rules); // Filter next node
    static bool filter_node_apply(xmlNodePtr node, std::vector<struct adr::adb_rule> *rules); // Apply filter
    static void nullGenericErrorFunc(void *ctxt, const char *msg, ...); // Empty error handler for libXML2
