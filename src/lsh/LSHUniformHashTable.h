...
namespace lsh
{
  ...
  template<class T>
	...
	{ return _filled.end (); }
      ...
      unsigned int filledSize () const { return _filled.size (); }
      ...
      virtual void LcomputeMKey (T te,
				 unsigned long int *Lmkeys) { std::cout << "[Warning]:Using undefined virtual root function LSHUniformHashTable::LcomputeMKey !\n";}

      virtual void LcomputeCKey (T te,
				 unsigned long int *Lckeys) { std::cout << "[Warning]:Using undefined virtual root function LSHUniformHashTable::LcomputeCKey !\n";}
